...
// lowercase all words -->
// consider punctuation -- look at charcode to see if it's a letter?
...
function buildWordCloud (wordString) {
  ...
  for (let i = 0; i < wordArray; i++) {
    for (let j = 0; j < wordArray[i].length; j++) {
      const wordToCheck = wordArry[i].toLowerCase();
      let wordToAddToCloud = '';

      if (wordToCheck.charCodeAt(j) >= 97 && wordToCheck.charCodeAt(j) <= 122) {
        wordToAdd += wordToCheck.charCodeAt(j);
      }
    }
